...
const template = (options) => {
  ...
  <head>
    ...
    <meta name="description" content="${JSON.parse(seoDescription)}">
    <meta name="viewport" content="width=device-width,initial-scale=1">
    <meta name="robots" content="index,follow">
    <meta name="google-site-verification" content="9P4FtusJunAS1pkZkQ_zCim0qapHt7_w9_Ly6WzrpkA">
    <meta name="theme-color" content="#ffffff">
    <meta name="msapplication-navbutton-color" content="#ffffff">
    <meta name="apple-mobile-web-app-status-bar-style" content="#ffffff">
    <title>${JSON.parse(seoTitle)}</title>
  ...
  <body><svg xmlns="http://www.w3.org/2000/svg" style="display: none;">
      ...
      <symbol id="share-icon-add-opinion" viewBox="0 0 48 48">
        <path
          d="M0 33c0 2.21 1.79 4 4 4h6v6.17c0 1.11 0.9 2 2 2 0.561 0 1.069-0.23 1.43-0.6l7.57-7.57h22c2.21 0 4-1.79 4-4l-0-25c0-2.21-1.79-4-4-4l-39 0c-2.21 0-4 1.79-4 4v25zM12 21.5c0-1.38 1.12-2.5 2.5-2.5s2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5-2.5-1.12-2.5-2.5zM21 21.5c0-1.38 1.12-2.5 2.5-2.5s2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5-2.5-1.12-2.5-2.5zM30 21.5c0-1.38 1.12-2.5 2.5-2.5s2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5-2.5-1.12-2.5-2.5z">
        </path>
      </symbol>
    </svg>
    <header class="header row"><a href="./">
    <svg class="logo"><use xlink:href="#logo"></use></svg>
        <div class="header-sentence">Bo życie to nie ilość wdechów czy wydechów,<br>to chwile, które zapierają nam dech.
        </div>
      </a><button class="hamburger" aria-label="nawigacja">
        <div class="hamburger-center"></div>
      </button></header>
    <main class="container">
      <article class="article">
        <header class="article__title">
          <h1>${JSON.parse(bodyTitle)}</h1><time class="article__date"><svg class="icon"
              viewBox="0 0 32 32">
              <path class="path1"
            ...
            </svg><small>${JSON.parse(bodyDate)}</small>
            <div class="share-button-temp" id="share-button-stick"></div>
          </time>
        </header>
        <div class="article__text">${JSON.parse(bodyText)}</div>
        <div class="article__content">
          ${images.map((image) => {
    const { img, alt, text } = JSON.parse(image);
    return `<figure>
            <div class="item__img">
              <picture>
                <source data-srcset="./images/lwow/576/${img}" media="(max-width: 576px)">
                <source data-srcset="./images/lwow/768/${img}" media="(max-width: 768px)">
                <source data-srcset="./images/lwow/992/${img}" media="(max-width: 992px)">
                <source data-srcset="./images/lwow/1200/${img}" media="(max-width: 1200px)">
                <img data-src="./images/lwow/1200/${img}" alt="">
                <noscript><img src="./images/lwow/1200/${img}" alt="${alt}"></noscript>
              </picture>
            </div>
            ${text ? `<figcaption>${text}</figcaption>` : ''}
          </figure>`;
  }).join('')}
          <div class="maps-title"></div>
        </div>
        <div class="share-button-temp" id="share-button-bottom"></div>
        <div class="disqus-contet row">
          <div id="disqus_thread"></div><noscript>Please enable JavaScript to view the <a
    ...
    <footer class="footer row text-center">
      <p class="text-small">Strona używa plików cookies. Brak zmiany ustawień przeglądarki oznacza zgodę na ich użycie.
      </p>
      <p>Copyright © ${new Date().getFullYear()} by kody.wig.pl</p>
    </footer><a class="scroll" href="#">
      <div class="back-to-top">Wróć na początek strony</div>
    </a>
    <script src="vendor/js/vendor.daf809d2e2f3ec658250.js"></script>
    <script src="vendor/js/styles.daf809d2e2f3ec658250.js"></script>
    <script src="vendor/js/article.daf809d2e2f3ec658250.js"></script>
  </body>
  
  </html>
  `;

  writeFile(`./html/${nameFolder}.html`, templateJson, (err) => {
